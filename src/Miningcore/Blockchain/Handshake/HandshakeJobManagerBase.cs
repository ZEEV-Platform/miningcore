...
public abstract class HandshakeJobManagerBase<TJob> : JobManagerBase<TJob>
{
    protected HandshakeJobManagerBase(
    ...
    protected HandshakePoolConfigExtra extraPoolConfig;
    ...
    protected virtual void SetupJobUpdates(CancellationToken ct)
    {
        ...
        if(extraPoolConfig?.BtStream == null)
        {
            ...
            var zmq = poolConfig.Daemons
                .Where(x => !string.IsNullOrEmpty(x.Extra.SafeExtensionDataAs<HandshakeDaemonEndpointConfigExtra>()?.ZmqBlockNotifySocket))
                .ToDictionary(x => x, x =>
                {
                    var extra = x.Extra.SafeExtensionDataAs<HandshakeDaemonEndpointConfigExtra>();
                    var topic = !string.IsNullOrEmpty(extra.ZmqBlockNotifyTopic?.Trim()) ? extra.ZmqBlockNotifyTopic.Trim() : HandshakeConstants.ZmqPublisherTopicBlockHash;
    ...
    protected virtual async Task ShowDaemonSyncProgressAsync(CancellationToken ct)
    {
        ...
        var info = await rpc.ExecuteAsync<BlockchainInfo>(logger, HandshakeCommands.GetBlockchainInfo, ct);
        ...
        if(info != null)
        {
            ...
            if(blockCount.HasValue)
            {
                ...
                var peerInfo = await rpc.ExecuteAsync<PeerInfo[]>(logger, HandshakeCommands.GetPeerInfo, ct);
    ...
    private async Task UpdateNetworkStatsAsync(CancellationToken ct)
    {
        try
        {
            var results = await rpc.ExecuteBatchAsync(logger, ct,
                new RpcRequest(HandshakeCommands.GetMiningInfo),
                new RpcRequest(HandshakeCommands.GetNetworkInfo),
                new RpcRequest(HandshakeCommands.GetNetworkHashPS)
            );
    ...
    protected async Task<SubmitResult> SubmitBlockAsync(Share share, string blockHex, CancellationToken ct)
    {
        var submitBlockRequest = hasSubmitBlockMethod
            ? new RpcRequest(HandshakeCommands.SubmitBlock, new[] { blockHex })
            : new RpcRequest(HandshakeCommands.GetBlockTemplate, new { mode = "submit", data = blockHex });

        var batch = new []
        {
            submitBlockRequest,
            new RpcRequest(HandshakeCommands.GetBlock, new[] { share.BlockHash })
        };
    ...
    {
        var response = await rpc.ExecuteAsync<DaemonInfo>(logger, HandshakeCommands.GetInfo, ct);
    ...
    {
        var info = await rpc.ExecuteAsync<DaemonInfo>(logger, HandshakeCommands.GetInfo, ct);
    ...
    private async Task UpdateNetworkStatsLegacyAsync(CancellationToken ct)
    {
        ...
        {
            var results = await rpc.ExecuteBatchAsync(logger, ct,
                new RpcRequest(HandshakeCommands.GetConnectionCount)
            );
    ...
    protected override async Task<bool> AreDaemonsHealthyAsync(CancellationToken ct)
    {
        ...
        var response = await rpc.ExecuteAsync<BlockchainInfo>(logger, HandshakeCommands.GetBlockchainInfo, ct);
    ...
    protected override async Task<bool> AreDaemonsConnectedAsync(CancellationToken ct)
    {
        ...
        var response = await rpc.ExecuteAsync<NetworkInfo>(logger, HandshakeCommands.GetNetworkInfo, ct);
    ...
    protected override async Task EnsureDaemonsSynchedAsync(CancellationToken ct)
    {
        ...
        do
        {
            var response = await rpc.ExecuteAsync<BlockTemplate>(logger,
                HandshakeCommands.GetBlockTemplate, ct, GetBlockTemplateParams());
    ...
    protected override async Task PostStartInitAsync(CancellationToken ct)
    {
        var requests = new[]
        {
            new RpcRequest(HandshakeCommands.ValidateAddress, new[] { poolConfig.Address }),
            new RpcRequest(HandshakeCommands.SubmitBlock),
            new RpcRequest(!hasLegacyDaemon ? HandshakeCommands.GetBlockchainInfo : HandshakeCommands.GetInfo),
            new RpcRequest(HandshakeCommands.GetDifficulty),
            new RpcRequest(HandshakeCommands.GetAddressInfo, new[] { poolConfig.Address }),
        };
        ...
        if(responses.Any(x => x.Error != null))
        {
            ...
            var errors = responses
                .Where((x, i) => x.Error != null &&
                    requests[i].Method != HandshakeCommands.SubmitBlock &&
                    requests[i].Method != HandshakeCommands.GetAddressInfo)
                .ToArray();
        ...
        if(!isPoS)
        {
            if(extraPoolConfig != null && extraPoolConfig.AddressType != HandshakeAddressType.Legacy)
    ...
    protected virtual IDestination AddressToDestination(string address, HandshakeAddressType? addressType)
    {
        if(!addressType.HasValue)
            return HandshakeUtils.AddressToDestination(address, network);

        switch(addressType.Value)
        {
            case HandshakeAddressType.BechSegwit:
                return HandshakeUtils.BechSegwitAddressToDestination(poolConfig.Address, network);

            default:
                return HandshakeUtils.AddressToDestination(poolConfig.Address, network);
        }
    }
    ...
    public virtual async Task<bool> ValidateAddressAsync(string address, CancellationToken ct)
    {
        ...
        var result = await rpc.ExecuteAsync<ValidateAddressResponse>(logger, HandshakeCommands.ValidateAddress, ct, new[] { address });
