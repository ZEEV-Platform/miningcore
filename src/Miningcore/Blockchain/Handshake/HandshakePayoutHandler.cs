    ...
    IPayoutHandler
{
    ...
    protected HandshakeDaemonEndpointConfigExtra extraPoolEndpointConfig;
    ...
    public virtual Task ConfigureAsync(ClusterConfig cc, PoolConfig pc, CancellationToken ct)
    {
        ...
        extraPoolEndpointConfig = pc.Extra.SafeExtensionDataAs<HandshakeDaemonEndpointConfigExtra>();
    ...
    public virtual async Task<Block[]> ClassifyBlocksAsync(IMiningPool pool, Block[] blocks, CancellationToken ct)
    {
        ...
        for(var i = 0; i < pageCount; i++)
        {
            ...
            var batch = page.Select(block => new RpcRequest(HandshakeCommands.GetTransaction,
    ...
    public virtual async Task PayoutAsync(IMiningPool pool, Balance[] balances, CancellationToken ct)
    {
        ...
        if(!(extraPoolConfig?.HasBrokenSendMany == true || poolConfig.Template is HandshakeCoinTemplate { HasBrokenSendMany: true }))
        {
            ...
            // send command
            tryTransfer:
            var result = await rpcClient.ExecuteAsync<string>(logger, HandshakeCommands.SendMany, ct, args);
            ...
            if(result.Error == null)
            {
                if(didUnlockWallet)
                {
                    ...
                    await rpcClient.ExecuteAsync<JToken>(logger, HandshakeCommands.WalletLock, ct);
                }
                ...
                if(string.IsNullOrEmpty(txId))
                    logger.Error(() => $"[{LogCategory}] {HandshakeCommands.SendMany} did not return a transaction id!");
                else
            ...
            {
                if(result.Error.Code == (int) HandshakeRPCErrorCode.RPC_WALLET_UNLOCK_NEEDED && !didUnlockWallet)
                {
                    if(!string.IsNullOrEmpty(extraPoolPaymentProcessingConfig?.WalletPassword))
                    {
                        logger.Info(() => $"[{LogCategory}] Unlocking wallet");

                        var unlockResult = await rpcClient.ExecuteAsync<JToken>(logger, HandshakeCommands.WalletPassphrase, ct, new[]
                        {
                        ...
                        if(unlockResult.Error == null)
                        {
                            ...
                            logger.Error(() => $"[{LogCategory}] {HandshakeCommands.WalletPassphrase} returned error: {result.Error.Message} code {result.Error.Code}");
                    }
                ...
                {
                    logger.Error(() => $"[{LogCategory}] {HandshakeCommands.SendMany} returned error: {result.Error.Message} code {result.Error.Code}");

                    NotifyPayoutFailure(poolConfig.Id, balances, $"{HandshakeCommands.SendMany} returned error: {result.Error.Message} code {result.Error.Code}", null);
                }
            }
        }
        ...
        else
        {
            ...
            await Parallel.ForEachAsync(amounts, parallelOptions, async (x, _ct) =>
            {
                ...
                await Guard(async () =>
                {
                    ...
                    var result = await rpcClient.ExecuteAsync<string>(logger, HandshakeCommands.SendToAddress, ct, new object[]
                    {
                        address,
                        amount,
                    });
                    ...
                    if(result.Error != null)
                        throw new Exception($"[{transferId}] {HandshakeCommands.SendToAddress} returned error: {result.Error.Message} code {result.Error.Code}");

                    if(string.IsNullOrEmpty(txId))
                        throw new Exception($"[{transferId}] {HandshakeCommands.SendToAddress} did not return a transaction id!");
                    else
