...
public class HandshakeJobManager : HandshakeJobManagerBase<HandshakeJob>
{
    ...
    protected async Task<RpcResponse<BlockTemplate>> GetBlockTemplateAsync(CancellationToken ct)
    {
        var result = await rpc.ExecuteAsync<BlockTemplate>(logger,
            HandshakeCommands.GetBlockTemplate, ct, extraPoolConfig?.GBTArgs ?? (object) GetBlockTemplateParams());
    ...
    private HandshakeJob CreateJob()
    {
        return new();
    }
    ...
    public virtual object[] GetSubscriberData(StratumConnection worker)
    {
        ...
        {
            context.ExtraNonce1,
            HandshakeConstants.ExtranoncePlaceHolderLength - ExtranonceBytes,
        };
