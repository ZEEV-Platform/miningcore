...
public class HandshakePool : PoolBase
{
    ...
    protected virtual async Task OnSubscribeAsync(StratumConnection connection, Timestamped<JsonRpcRequest> tsRequest)
    {
        ...
        var data = new object[]
        {
            new object[]
            {
                new object[] { HandshakeStratumMethods.SetDifficulty, connection.ConnectionId },
                new object[] { HandshakeStratumMethods.MiningNotify, connection.ConnectionId }
            }
        }
        ...
        // send intial update
        await connection.NotifyAsync(HandshakeStratumMethods.SetDifficulty, new object[] { context.Difficulty });
        await connection.NotifyAsync(HandshakeStratumMethods.MiningNotify, currentJobParams);
    }
    ...
    private async Task OnConfigureMiningAsync(StratumConnection connection, Timestamped<JsonRpcRequest> tsRequest)
    {
        ...
        if(extensions != null)
        {
            foreach(var extension in extensions)
            {
                switch(extension)
                {
                    case HandshakeStratumExtensions.VersionRolling:
                        ConfigureVersionRolling(connection, context, extensionParams, result);
                        break;

                    case HandshakeStratumExtensions.MinimumDiff:
    ...
    private void ConfigureVersionRolling(StratumConnection connection, HandshakeWorkerContext context,
        IReadOnlyDictionary<string, JToken> extensionParams, Dictionary<string, object> result)
    {
        //var requestedBits = extensionParams[HandshakeStratumExtensions.VersionRollingBits].Value<int>();
        var requestedMask = HandshakeConstants.VersionRollingPoolMask;

        if(extensionParams.TryGetValue(HandshakeStratumExtensions.VersionRollingMask, out var requestedMaskValue))
            requestedMask = uint.Parse(requestedMaskValue.Value<string>(), NumberStyles.HexNumber);

        // Compute effective mask
        context.VersionRollingMask = HandshakeConstants.VersionRollingPoolMask & requestedMask;

        // enabled
        result[HandshakeStratumExtensions.VersionRolling] = true;
        result[HandshakeStratumExtensions.VersionRollingMask] = context.VersionRollingMask.Value.ToStringHex8();

        logger.Info(() => $"[{connection.ConnectionId}] Using version-rolling mask {result[HandshakeStratumExtensions.VersionRollingMask]}");
    }

    private void ConfigureMinimumDiff(StratumConnection connection, HandshakeWorkerContext context,
        IReadOnlyDictionary<string, JToken> extensionParams, Dictionary<string, object> result)
    {
        var requestedDiff = extensionParams[HandshakeStratumExtensions.MinimumDiffValue].Value<double>();
        ...
        if(requestedDiff > poolEndpoint.Difficulty)
        {
            ...
            // enabled
            result[HandshakeStratumExtensions.MinimumDiff] = true;
        }
    }
    ...
    {
        var multiplier = HandshakeConstants.Pow2x32;
    ...
    protected override async Task SetupJobManager(CancellationToken ct)
    {
        manager = ctx.Resolve<HandshakeJobManager>(
            new TypedParameter(typeof(IExtraNonceProvider), new HandshakeExtraNonceProvider(poolConfig.Id, clusterConfig.InstanceId)));
        ...
        Timestamped<JsonRpcRequest> tsRequest, CancellationToken ct)
    {
        ...
        try
        {
            ...
            {
                case HandshakeStratumMethods.Subscribe:
                    await OnSubscribeAsync(connection, tsRequest);
                    break;

                case HandshakeStratumMethods.Authorize:
                    await OnAuthorizeAsync(connection, tsRequest, ct);
                    break;

                case HandshakeStratumMethods.SubmitShare:
                    await OnSubmitAsync(connection, tsRequest, ct);
                    break;

                case HandshakeStratumMethods.SuggestDifficulty:
                    await OnSuggestDifficultyAsync(connection, tsRequest);
                    break;

                case HandshakeStratumMethods.MiningConfigure:
                    await OnConfigureMiningAsync(connection, tsRequest);
                    // ignored
                    break;

                case HandshakeStratumMethods.ExtraNonceSubscribe:
                    await connection.RespondAsync(true, request.Id);
                    break;

                case HandshakeStratumMethods.GetTransactions:
                    // ignored
                    break;

                case HandshakeStratumMethods.MiningMultiVersion:
                    // ignored
                    break;
    ...
    protected override async Task OnVarDiffUpdateAsync(StratumConnection connection, double newDiff, CancellationToken ct)
    {
        ...
        {
            await connection.NotifyAsync(HandshakeStratumMethods.SetDifficulty, new object[] { connection.Context.Difficulty });
            await connection.NotifyAsync(HandshakeStratumMethods.MiningNotify, currentJobParams);
        }
    }
