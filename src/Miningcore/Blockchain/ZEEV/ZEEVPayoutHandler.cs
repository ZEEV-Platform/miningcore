...
public class ZEEVPayoutHandler : PayoutHandlerBase,
    IPayoutHandler
{
    public ZEEVPayoutHandler(
    ...
    protected ZEEVDaemonEndpointConfigExtra extraPoolEndpointConfig;
    protected ZEEVPoolPaymentProcessingConfigExtra extraPoolPaymentProcessingConfig;

    protected override string LogCategory => "ZEEV Payout Handler";
    ...
    public virtual Task ConfigureAsync(ClusterConfig cc, PoolConfig pc, CancellationToken ct)
    {
        ...
        extraPoolEndpointConfig = pc.Extra.SafeExtensionDataAs<ZEEVDaemonEndpointConfigExtra>();
        extraPoolPaymentProcessingConfig = pc.PaymentProcessing.Extra.SafeExtensionDataAs<ZEEVPoolPaymentProcessingConfigExtra>();

        logger = LogUtil.GetPoolScopedLogger(typeof(ZEEVPayoutHandler), pc);
    ...
    public virtual async Task<Block[]> ClassifyBlocksAsync(IMiningPool pool, Block[] blocks, CancellationToken ct)
    {
        ...
        if(coin is ZEEVCoinTemplate ZEEVTemplate)
            minConfirmations = extraPoolEndpointConfig?.MinimumConfirmations ?? ZEEVTemplate.CoinbaseMinConfimations ?? ZEEVConstants.CoinbaseMinConfimations;
        else
            minConfirmations = extraPoolEndpointConfig?.MinimumConfirmations ?? ZEEVConstants.CoinbaseMinConfimations;
        ...
        for(var i = 0; i < pageCount; i++)
        {
            ...
            var batch = page.Select(block => new RpcRequest(ZEEVCommands.GetTransaction,
    ...
    public virtual async Task PayoutAsync(IMiningPool pool, Balance[] balances, CancellationToken ct)
    {
        ...
        if(!(extraPoolConfig?.HasBrokenSendMany == true || poolConfig.Template is ZEEVCoinTemplate { HasBrokenSendMany: true }))
        {
            if(extraPoolPaymentProcessingConfig?.MinersPayTxFees == true)
            {
                var subtractFeesFrom = amounts.Keys.ToArray();

                if(!poolConfig.Template.As<ZEEVCoinTemplate>().HasMasterNodes)
                {
                    args = new object[]
                    {
            ...
            if(result.Error == null)
            {
                if(didUnlockWallet)
                {
                    ...
                    await rpcClient.ExecuteAsync<JToken>(logger, ZEEVCommands.WalletLock, ct);
                }
                ...
                if(string.IsNullOrEmpty(txId))
                    logger.Error(() => $"[{LogCategory}] {ZEEVCommands.SendMany} did not return a transaction id!");
                else
            ...
            {
                if(result.Error.Code == (int) ZEEVRPCErrorCode.RPC_WALLET_UNLOCK_NEEDED && !didUnlockWallet)
                {
                    if(!string.IsNullOrEmpty(extraPoolPaymentProcessingConfig?.WalletPassword))
                    {
                        ...
                        if(unlockResult.Error == null)
                        {
                            ...
                            logger.Error(() => $"[{LogCategory}] {ZEEVCommands.WalletPassphrase} returned error: {result.Error.Message} code {result.Error.Code}");
                    }
                ...
                {
                    logger.Error(() => $"[{LogCategory}] {ZEEVCommands.SendMany} returned error: {result.Error.Message} code {result.Error.Code}");

                    NotifyPayoutFailure(poolConfig.Id, balances, $"{ZEEVCommands.SendMany} returned error: {result.Error.Message} code {result.Error.Code}", null);
                }
            }
        }
        ...
        else
        {
            ...
            await Parallel.ForEachAsync(amounts, parallelOptions, async (x, _ct) =>
            {
                ...
                await Guard(async () =>
                {
                    ...
                    var result = await rpcClient.ExecuteAsync<string>(logger, ZEEVCommands.SendToAddress, ct, new object[]
                    {
                        address,
                        amount,
                    });
                    ...
                    if(result.Error != null)
                        throw new Exception($"[{transferId}] {ZEEVCommands.SendToAddress} returned error: {result.Error.Message} code {result.Error.Code}");

                    if(string.IsNullOrEmpty(txId))
                        throw new Exception($"[{transferId}] {ZEEVCommands.SendToAddress} did not return a transaction id!");
                    else
