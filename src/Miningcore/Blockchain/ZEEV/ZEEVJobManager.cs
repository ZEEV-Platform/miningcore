...
public class ZEEVJobManager : ZEEVJobManagerBase<ZEEVJob>
{
    public ZEEVJobManager(
    ...
    protected async Task<RpcResponse<BlockTemplate>> GetBlockTemplateAsync(CancellationToken ct)
    {
        var result = await rpc.ExecuteAsync<BlockTemplate>(logger,
            ZEEVCommands.GetBlockTemplate, ct, extraPoolConfig?.GBTArgs ?? (object) GetBlockTemplateParams());
    ...
    public virtual object[] GetSubscriberData(StratumConnection worker)
    {
        ...
        {
            context.ExtraNonce1,
            ZEEVConstants.ExtranoncePlaceHolderLength - ExtranonceBytes,
        };
