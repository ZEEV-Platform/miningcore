...
[CoinFamily(CoinFamily.ZEEV)]
public class ZEEVPool : PoolBase
{
    public ZEEVPool(IComponentContext ctx,
    ...
    protected ZEEVJobManager manager;
    private ZEEVCoinTemplate coin;
    ...
    protected virtual async Task OnSubscribeAsync(StratumConnection connection, Timestamped<JsonRpcRequest> tsRequest)
    {
        ...
        var context = connection.ContextAs<ZEEVWorkerContext>();
        ...
        var data = new object[]
        {
            new object[]
            {
                new object[] { ZEEVStratumMethods.SetDifficulty, connection.ConnectionId },
                new object[] { ZEEVStratumMethods.MiningNotify, connection.ConnectionId }
            }
        }
        ...
        // send intial update
        await connection.NotifyAsync(ZEEVStratumMethods.SetDifficulty, new object[] { context.Difficulty });
        await connection.NotifyAsync(ZEEVStratumMethods.MiningNotify, currentJobParams);
    }
    ...
    private async Task OnConfigureMiningAsync(StratumConnection connection, Timestamped<JsonRpcRequest> tsRequest)
    {
        ...
        if(extensions != null)
        {
            foreach(var extension in extensions)
            {
                switch(extension)
                {
                    case ZEEVStratumExtensions.VersionRolling:
                        ConfigureVersionRolling(connection, context, extensionParams, result);
                        break;

                    case ZEEVStratumExtensions.MinimumDiff:
    ...
    private void ConfigureVersionRolling(StratumConnection connection, ZEEVWorkerContext context,
        IReadOnlyDictionary<string, JToken> extensionParams, Dictionary<string, object> result)
    {
        //var requestedBits = extensionParams[ZEEVStratumExtensions.VersionRollingBits].Value<int>();
        var requestedMask = ZEEVConstants.VersionRollingPoolMask;

        if(extensionParams.TryGetValue(ZEEVStratumExtensions.VersionRollingMask, out var requestedMaskValue))
            requestedMask = uint.Parse(requestedMaskValue.Value<string>(), NumberStyles.HexNumber);

        // Compute effective mask
        context.VersionRollingMask = ZEEVConstants.VersionRollingPoolMask & requestedMask;

        // enabled
        result[ZEEVStratumExtensions.VersionRolling] = true;
        result[ZEEVStratumExtensions.VersionRollingMask] = context.VersionRollingMask.Value.ToStringHex8();

        logger.Info(() => $"[{connection.ConnectionId}] Using version-rolling mask {result[ZEEVStratumExtensions.VersionRollingMask]}");
    }

    private void ConfigureMinimumDiff(StratumConnection connection, ZEEVWorkerContext context,
        IReadOnlyDictionary<string, JToken> extensionParams, Dictionary<string, object> result)
    {
        var requestedDiff = extensionParams[ZEEVStratumExtensions.MinimumDiffValue].Value<double>();
        ...
        if(requestedDiff > poolEndpoint.Difficulty)
        {
            ...
            // enabled
            result[ZEEVStratumExtensions.MinimumDiff] = true;
        }
    }
    ...
    {
        var multiplier = ZEEVConstants.Pow2x32;
    ...
    {
        manager = ctx.Resolve<ZEEVJobManager>(
            new TypedParameter(typeof(IExtraNonceProvider), new ZEEVExtraNonceProvider(poolConfig.Id, clusterConfig.InstanceId)));
        ...
        Timestamped<JsonRpcRequest> tsRequest, CancellationToken ct)
    {
        ...
        try
        {
            ...
            {
                case ZEEVStratumMethods.Subscribe:
                    await OnSubscribeAsync(connection, tsRequest);
                    break;

                case ZEEVStratumMethods.Authorize:
                    await OnAuthorizeAsync(connection, tsRequest, ct);
                    break;

                case ZEEVStratumMethods.SubmitShare:
                    await OnSubmitAsync(connection, tsRequest, ct);
                    break;

                case ZEEVStratumMethods.SuggestDifficulty:
                    await OnSuggestDifficultyAsync(connection, tsRequest);
                    break;

                case ZEEVStratumMethods.MiningConfigure:
                    await OnConfigureMiningAsync(connection, tsRequest);
                    // ignored
                    break;

                case ZEEVStratumMethods.ExtraNonceSubscribe:
                    await connection.RespondAsync(true, request.Id);
                    break;

                case ZEEVStratumMethods.GetTransactions:
                    // ignored
                    break;

                case ZEEVStratumMethods.MiningMultiVersion:
                    // ignored
                    break;
    ...
    protected override async Task OnVarDiffUpdateAsync(StratumConnection connection, double newDiff, CancellationToken ct)
    {
        ...
        {
            await connection.NotifyAsync(ZEEVStratumMethods.SetDifficulty, new object[] { connection.Context.Difficulty });
            await connection.NotifyAsync(ZEEVStratumMethods.MiningNotify, currentJobParams);
        }
    }
